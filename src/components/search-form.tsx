...
export function SearchForm({
  ...
  function handleSubmit(e: FormEvent) {
    ...
    if (!gameName || !tagLine) {
      setError("Format is gameName#tagLine, e.g. Faker#KR1");
      return;
    }
    setError(null);
    ...
    <form onSubmit={handleSubmit} className="w-full max-w-xl">
      <div className="flex flex-col gap-3 sm:flex-row">
        <input
          ...
          className="flex-1 rounded-xl border border-white/15 bg-white/5 px-4 py-3 text-base text-zinc-100 placeholder:text-zinc-500 outline-none focus:border-violet-400/60 focus:ring-2 focus:ring-violet-400/30"
        />
        <select
          value={platform}
          onChange={(e) => setPlatform(e.target.value as Platform)}
          className="rounded-xl border border-white/15 bg-white/5 px-3 py-3 text-sm text-zinc-100 outline-none focus:border-violet-400/60"
        >
          {PLATFORMS.map((p) => (
